...
class StrikeService:
        ...
        try:
            ...
            try:
                ...
                if strike_info.get('strike_type') == 'no_action':
                    self.logger.info(f"No action taken for {card_uid}: {strike_info.get('reason', 'Unknown reason')}")
                    return strike_info
    ...
    def _determine_and_apply_strike(self, session, user: User, violation: Dict) -> Dict:
        try:
            ...
            violation_locked_at = violation.get('locked_at')
            if isinstance(violation_locked_at, datetime):
                if violation_locked_at.tzinfo is None:
                    violation_locked_at = violation_locked_at.replace(tzinfo=timezone.utc)
                violation_date_str = violation_locked_at.isoformat()
            else:
                violation_date_str = current_time.isoformat()
                violation_locked_at = current_time
            ...
            if not has_strike_1 and user.last_violation_date is not None:
                last_violation = user.last_violation_date

                if last_violation.tzinfo is None:
                    last_violation = last_violation.replace(tzinfo=timezone.utc)

                if violation_locked_at <= last_violation:
                    self.logger.info(f"🔄 DUPLICATE VIOLATION: User {user.card_uid} - Current violation ({violation_locked_at}) is not newer than last violation ({last_violation}). This appears to be the same uncorrected violation from a previous Strike 3. No action taken.")

                    return {
                        'strike_type': 'no_action',
                        'violation_date': violation_date_str,
                        'location': violation.get('location', 'Unknown'),
                        'lock_id': violation.get('lock_id', 'Unknown'),
                        'counter': current_counter,
                        'reason': 'Violation is not newer than last recorded violation (uncorrected Strike 3)'
                }

            if not has_strike_1:
                user.strike_1_date = current_time  # type: ignore[assignment]
                strike_info['strike_type'] = 'strike_1'
                self.logger.info(f"Applied Strike 1 to user {user.card_uid}")

            elif not has_strike_2:
                user.strike_2_date = current_time  # type: ignore[assignment]
                ...
                user.counter = current_counter + 1  # type: ignore[assignment]
    ...
    def is_user_in_cooldown(self, card_uid: str) -> bool:
        try:
            ...
            try:
                ...
                if not user:
                    self.logger.debug(f"No user found for {card_uid}")
                    return False

                if user.last_violation_date is None:
                    self.logger.debug(f"User {card_uid} has no last_violation_date")
                    return False
                ...
                self.logger.debug(f"DEBUG cooldown check for {card_uid}:")
                self.logger.debug(f"  - current_time: {current_time} (type: {type(current_time)}, tzinfo: {current_time.tzinfo})")
                self.logger.debug(f"  - last_violation: {last_violation} (type: {type(last_violation)}, tzinfo: {getattr(last_violation, 'tzinfo', 'N/A')})")

                # Convert if needed
                if last_violation.tzinfo is None:
                    self.logger.debug(f"  - Converting naive last_violation to UTC")
                    last_violation = last_violation.replace(tzinfo=timezone.utc)
                    # Update database immediately
                    user.last_violation_date = last_violation  # type: ignore[assignment]
                    session.commit()

                # Calculate cooldown cutoff time
                cooldown_cutoff = current_time - timedelta(hours=self.config.STRIKE_COOLDOWN_HOURS)

                self.logger.debug(f"  - cooldown_cutoff: {cooldown_cutoff}")
                self.logger.debug(f"  - is {last_violation} >= {cooldown_cutoff}?")

                # Check if last violation was within cooldown period
                in_cooldown = last_violation >= cooldown_cutoff

                self.logger.debug(f"  - in_cooldown: {in_cooldown}")
            ...
            self.logger.error(f"Error checking cooldown for {card_uid}: {str(e)}", exc_info=True)
            return False
    ...
    def validate_strike_data(self, card_uid: str) -> bool:
        try:
            ...
            try:
                ...
                if has_strike_1 and has_strike_2 and user.strike_1_date is not None and user.strike_2_date is not None:
